...
def min_node(main_player: int, state: Game, depth: int) -> tuple:
    min_eval = float('inf')
...
def max_node(main_player: int, state: Game, depth: int) -> tuple:
    max_eval = float('-inf')
    possible_actions = state.get_possible_actions()
    for action in possible_actions:
        # Take the action in a copy of the state
...
def chance_node(main_player: int, state: Game, depth: int) -> tuple:
    expected_utility = 0
...
def expectiminimax(main_player: int, state: Game, depth: int=4, chance: bool=False) -> tuple:
